...
export function EndpointPage() {
  ...
  if (!endpoint) {
    return (
      <div className="animate-fade-up py-16 text-center">
        ...
        <p className="mt-1 text-sm text-slate-400">It may have been renamed or removed.</p>
  ...
  return (
    <div className="animate-fade-up space-y-6 pb-16">
      <Link
        to="/docs"
        className="inline-flex items-center gap-1 text-[13px] font-medium text-slate-400 hover:text-slate-200"
      >
      ...
      <div>
        <div className="mb-2 flex items-center gap-2">
          ...
          <span className="text-[12px] font-medium capitalize text-slate-400">{endpoint.category}</span>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="card p-5">
          {endpoint.methods.length > 1 && (
            <div className="mb-4 inline-flex rounded-full bg-white/10 p-0.5 text-[13px] font-semibold">
              {endpoint.methods.map((m) => (
                <button
                  ...
                  className={`rounded-full px-3.5 py-1.5 transition-colors ${
                    method === m
                      ? 'bg-surface-darkcard text-white shadow-ios-sm'
                      : 'text-slate-400'
                  }`}
                >
                  {m}
          ...
          <details className="group mt-5">
            <summary className="cursor-pointer text-[12.5px] font-medium text-slate-400 hover:text-slate-300">
              View as curl
            </summary>
            <pre className="mt-2 overflow-x-auto rounded-xl bg-[#0d1420] p-3 font-mono text-[12px] leading-relaxed text-slate-300">
              {buildCurl(fullUrl, method, values)}
            </pre>
          </details>
        </div>
