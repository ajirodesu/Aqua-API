  ...
  return (
    <nav className="flex h-full flex-col" aria-label="API categories">
      ...
      <NavLink
        ...
        onClick={onNavigate}
        className={({ isActive }) =>
          `mx-3 mb-2 flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${
            isActive
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-6">
          ...
          return (
            <div key={bucket.name} className="mb-1">
              <button
                ...
                className="flex w-full items-center gap-1.5 rounded-lg px-2.5 py-2 text-[11px] font-bold uppercase tracking-wider text-slate-500 hover:text-slate-300"
              >
                <ChevronDown
                        ...
                        <NavLink
                          ...
                          className={({ isActive }) =>
                            `group flex items-center gap-2 rounded-lg px-3 py-2 text-[13px] transition-colors ${
                              isActive
                                ? 'bg-aqua-500/10 font-semibold text-aqua-300'
                                : 'text-slate-300 hover:bg-white/5'
                            }`
                          }
                        >
        ...
        {filtered.length === 0 && (
          <p className="px-3 py-6 text-center text-sm text-slate-400">No endpoints match "{query}".</p>
        )}
      </div>
    </nav>
  );
