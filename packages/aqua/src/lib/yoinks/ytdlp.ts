/**
 * Ported from yoinks (https://github.com/pablostanley/yoinks)
 * `src/lib/ytdlp.ts` — the actual engine behind yoinks, not the npm package.
 * This is vendored directly (per project requirements) rather than pulled in
 * as a dependency, so Aqua owns the code that talks to yt-dlp.
 *
 * Two changes from the original CLI source:
 *   1. The module-level `activeChild` + `process.on('exit', ...)` cleanup
 *      hook was removed. That pattern assumes a single in-flight download
 *      (true for a terminal app, false for an API server handling several
 *      requests at once) — each call now tracks its own child process and
 *      relies on the AbortSignal passed straight to `spawn()`, which Node
 *      already uses to kill the child on abort.
 *   2. The binary cache directory lives under `.cache/yoinks/bin` inside the
 *      aqua package instead of `~/.yoinks/bin`, so it doesn't collide with a
 *      real yoinks CLI install on the same machine.
 */
...
// packages/aqua/.cache/yoinks/bin
const YOINKS_DIR = path.resolve(__dirname, '..', '..', '..', '.cache', 'yoinks', 'bin');
...
export async function probe(ytdlp: string, url: string, signal?: AbortSignal): Promise<ProbeResult> {
  ...
  const infoJsonPath = path.join(os.tmpdir(), `aqua-yoinks-info-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
