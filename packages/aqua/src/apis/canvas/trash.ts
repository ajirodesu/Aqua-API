...
export const meta: ApiMeta = {
  name: 'Trash',
  desc: 'Generate a trash meme overlay with a blurred image',
  method: ['get', 'post'],
  category: 'canvas',
  params: [
    {
      name: 'image',
      desc: 'URL or uploaded image to put in the trash',
...
export const onStart: ApiHandler = async ({ req, res }) => {
  ...
  try {
    const bgUrl = 'https://i.postimg.cc/VvBCXdtq/trash.png';
    ...
    // Create canvas based on background image dimensions
    ...
    // Target overlay size and position
    const targetX = 309;
    const targetY = 0;
    const targetW = 309;
    const targetH = 309;
    ...
    // 1. Draw base trash frame
    ctx.drawImage(bgImage, 0, 0);

    // 2. Draw user overlay image at (309, 0) with 5px blur (replicating image.blur(5) from Jimp)
    ctx.save();
    ctx.filter = 'blur(5px)';
