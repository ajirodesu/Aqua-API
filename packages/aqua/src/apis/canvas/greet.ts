  ...
  try {
    ...
    const backgroundImg =
      backgroundInput === 'dynamic'
        ? await loadDynamicBackground(body?.backgroundTheme, cfg.width, cfg.height, `greet_${platform}_bg`, loadRemoteImage)
        : backgroundInput
          ? await loadRemoteImage(backgroundInput, `greet_${platform}_bg`)
          : null;
