...
export const meta: ApiMeta = {
  name: 'Egypt Karaba',
  desc: 'Generate an Egypt Karaba meme overlay with an image avatar',
  method: ['get', 'post'],
  category: 'canvas',
  params: [
    {
      name: 'image',
      desc: 'URL or uploaded image for the Egypt meme target',
...
export const onStart: ApiHandler = async ({ req, res }) => {
  ...
  try {
    const bgUrl = 'https://i.postimg.cc/8PSdsVvk/karaba.png';

    // Fetch background template and user avatar concurrently
    const [karabaTemplate, avatarImage] = await Promise.all([
      loadImage(bgUrl),
      loadImage(resolveImageSource(image)),
    ]);

    // Create canvas matching natural background template dimensions
    const canvas = createCanvas(karabaTemplate.width, karabaTemplate.height);
    ...
    // Target avatar position and dimensions (130, 44) with size 130x130
    const targetX = 130;
    const targetY = 44;
    const targetW = 130;
    const targetH = 130;
    ...
    // 2. Aspect ratio crop (cover mode) so avatar fits cleanly without distortion
    ...
    // 3. Draw user avatar UNDER the template at (130, 44) with size 130x130
    ctx.drawImage(avatarImage, sx, sy, sWidth, sHeight, targetX, targetY, targetW, targetH);

    // 4. Draw Karaba template layer ON TOP
    ctx.drawImage(karabaTemplate, 0, 0, karabaTemplate.width, karabaTemplate.height);
